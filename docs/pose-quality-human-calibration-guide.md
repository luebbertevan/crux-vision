...
| Analyzed frame seeker | The ordinary previous/next seeker remains visible in calibration; its center readout becomes an editable analyzed-sample number and shows the exact presentation timestamp. | Use this for repeatable same-frame smoothing checks. It is exact for analyzed pose frames, not unanalyzed source frames. |
